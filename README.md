...
Version 0.6.0 adds **Enemy Class Icon over Nameplate**. While the arena runtime is active, a large class icon is anchored above each currently visible Blizzard nameplate whose unit GUID matches one of the mapped `arena1` through `arena5` opponent GUIDs. Enemy pets whose GUID matches `arenapet1` through `arenapet5` show that unit's own Blizzard portrait instead of an owner class icon. Totems, guardians, NPCs, friendly players, unrelated enemy players, and pets without an arena-pet GUID match do not receive icons.
