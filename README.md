...
The preferred source is a positive, accessible `AuraData.points[1]` from a canonical PW:S aura returned by `C_UnitAuras.GetAuraDataByIndex`; legacy `UnitAura` remains the aura-detection fallback. Anniversary builds can expose `points[1]` as a zero placeholder or secret value that cannot be validated, either of which is treated as unavailable rather than displayed as a real remaining amount. `UnitGetTotalAbsorbs("player")` is then consulted, but only after PW:S itself has been confirmed. This total fallback is explicitly lower confidence and can include other simultaneous absorb effects; the addon does not attempt to subtract unknown absorbs. If PW:S is absent, a positive total absorb from another effect never shows the frame. If both accessible sources return zero while the aura is still present, the display becomes `?` until the aura disappears or a positive amount becomes available.
