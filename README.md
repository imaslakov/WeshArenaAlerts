...
Version 0.6.0 adds **Class / Pet Icons over Nameplates**. While the arena runtime is active, a large class icon is anchored above each currently visible Blizzard nameplate whose unit GUID matches one of the mapped `arena1` through `arena5` opponent GUIDs. Enemy pets matched through `arenapet1` through `arenapet5`, and friendly pets matched through `pet` or `partypet1` through `partypet4`, show that unit's own Blizzard portrait instead of an owner class icon. A compact event-driven health bar appears immediately above every matched pet portrait. Totems, unrelated NPCs and players, and pets without a supported pet-unit GUID match do not receive icons.
